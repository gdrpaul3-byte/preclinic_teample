...
```
web/                단일 페이지 앱 (index.html + styles.css + app.js + data.json)
web/assets/gifs/    각 조 주제 GIF (group-<n>.gif) + placeholder.gif
scripts/
...
# Wrote web/data.json: 10 groups, 52 members, 5 leaders set, 1 topics confirmed
```

마크다운 원본이 갱신될 때마다 다시 실행하면 `web/data.json`이 덮어써집니다.
...
산출물은 `web/assets/gifs/group-<n>.gif`로 저장되어 페이지가 자동으로 이를 보여줍니다.
